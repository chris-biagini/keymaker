...
Sparse Matrix rain: up to ~10 active columns on a glyph grid, each a falling
head glyph with a dithered trail (trailing glyphs render from a
checkerboard-masked "dim" bank — one fade stage, the 1-bit stand-in for 50%
gray — then vanish from the tail). The glyph sheet is built once at boot
from `terminalio.FONT` glyphs plus their masked dim variants; no committed
rain asset. *(Amended 2026-08-23 during plan review: boot-built sheet and a
single dim stage replace the earlier pre-baked-strip / multi-stage-fade
wording — simpler, no binary asset, same look at arm's length.)*

- Advances on the shared frame clock (§6) at a bench-tunable divisor of it
  (planned: every 2nd frame, ~5 fps — a full-height fall at 10 fps was too
  fast). Each frame touches only each column's head cell and oldest-trail
  cell — never a full-screen clear.
...
- Numerals are white-on-black at full contrast. No textures in v1 (parked —
  see §9).
- Digits are generated by `tools/gen_digits.py` as chunky seven-segment
  geometry on a 14×24 grid — deterministic pure-integer rendering, no font
  files — committed as row bitmasks with a test asserting committed bytes
  match the generator (the Coach WAV pattern). The firmware builds bitmaps
  at boot: small (14×24) direct, big (28×48) by 2× nearest-neighbor
  upscale. *(Amended 2026-08-23 during plan review: replaces the earlier
  "two pre-baked sizes from a chosen bold face" wording — one source of
  truth, machine-independent generation.)*
