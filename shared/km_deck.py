...
SLOTS_PER_PAGE = 12
FOCUS_MAX = 40         # NOT the 21-column screen width -- see spec 5.2. Trimming
                       # to the screen width would make km_text.marquee's scroll
                       # unreachable, since marquee returns early whenever the
                       # text already fits the display width.
...
class Deck:
    ...
    def message(self, page, colors, focused=None, bells=(), name_max=14, ws_max=12):
        ...
        blank the pad. The knob only pages now, so no mode is carried here.
        """
        bells = set(bells)
